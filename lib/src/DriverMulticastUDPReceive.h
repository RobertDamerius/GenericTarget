#ifndef DRIVER_MULTICAST_UDP_RECEIVE_H
#define DRIVER_MULTICAST_UDP_RECEIVE_H
 ...
 */
extern void CreateDriverMulticastUDPReceive(uint16_t port, uint8_t* ipInterface, uint8_t* ipGroup, uint32_t rxBufferSize, int32_t prioritySocket, int32_t priorityThread, uint8_t ttl, const uint32_t numBuffers, const uint32_t bufferStrategy, uint8_t* ipFilter, uint8_t countAsDiscarded);

/**
 *  @brief Delete the multicast UDP receive driver.
 */
extern void DeleteDriverMulticastUDPReceive(void);
 ...
 */
extern void OutputDriverMulticastUDPReceive(uint16_t port, uint32_t rxBufferSize, uint32_t maxNumMessages, uint16_t* sources, uint8_t* bytes, uint32_t* lengths, double* timestamps, uint32_t* numMessagesReceived, uint32_t* numMessagesDiscarded);


#endif /* DRIVER_MULTICAST_UDP_RECEIVE_H */
