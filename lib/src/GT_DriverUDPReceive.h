#ifndef GT_DRIVER_UDP_RECEIVE_H
#define GT_DRIVER_UDP_RECEIVE_H
 ...
 */
extern void GT_CreateDriverUDPReceive(uint16_t port, uint8_t* ipInterface, uint32_t rxBufferSize, int32_t prioritySocket, int32_t priorityThread, const uint32_t numBuffers, const uint32_t bufferStrategy, uint8_t* ipFilter, uint8_t countAsDiscarded);

/**
 *  @brief Delete the UDP receive driver.
 */
extern void GT_DeleteDriverUDPReceive(void);
 ...
 */
extern void GT_OutputDriverUDPReceive(uint16_t port, uint32_t rxBufferSize, uint32_t maxNumMessages, uint16_t* sources, uint8_t* bytes, uint32_t* lengths, double* timestamps, uint32_t* numMessagesReceived, uint32_t* numMessagesDiscarded);


#endif /* GT_DRIVER_UDP_RECEIVE_H */
