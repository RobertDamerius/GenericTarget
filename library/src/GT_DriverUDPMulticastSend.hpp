 ...
 * @param[in] interfaceSendName Bytes of the string representing the interface name via which to send if interfaceSendNameLength is not zero.
 ...
 */
extern void GT_DriverUDPMulticastSendInitialize(uint16_t port, uint8_t* ipGroup, uint8_t* interfaceSendIP, uint8_t* interfaceSendName, uint32_t interfaceSendNameLength, int32_t prioritySocket, uint8_t ttl, uint8_t allowBroadcast);
